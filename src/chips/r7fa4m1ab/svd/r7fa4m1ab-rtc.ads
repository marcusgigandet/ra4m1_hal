...
package R7FA4M1AB.RTC is
   ...
   for BCNT2AER_Register use record
      ENB      at 0 range 0 .. 7;
      Reserved at 0 range 8 .. 15;
   end record;
...
--  prescaler every 10 seconds.
   ...
   type RTCCR_Reserved_Field_Array is array (1 .. 2)
     of RTCCR_Reserved_Element
     with Component_Size => 1, Size => 2;

   --  Type definition for RTCCR_Reserved
   type RTCCR_Reserved_Field_1
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
   ...
   for RTCCR_Reserved_Field_1 use record
      Val at 0 range 0 .. 1;
      Arr at 0 range 0 .. 1;
   end record;
   ...
   --  Realtime Clock
   type RTC_Peripheral
     (Discriminent : RTC_Disc := View_R64CNT)
   is record
